...
class StreakInfoView(discord.ui.View):
    ...
    async def streak_info(self, interaction: discord.Interaction, button: discord.ui.Button):
        explanation = (
            "**فكرة الستريك 🔥**\n"
            f"لازم ترسل {MESSAGES_REQUIRED} رسائل باليوم بهاد الروم عشان يُحسب إلك يوم ستريك.\n"
            "الستريك بيتجدد كل يوم الساعة 12 الظهر.\n"
            "إذا كملت وصلت للعدد المطلوب يوم بعد يوم، الستريك بيزيد رقم رقم بلا حد.\n"
            "بس إذا فوّت يوم ولا وصلت للعدد المطلوب، الستريك بيرجع صفر ولازم تبلش من جديد 💪"
        )
        ...
        if not user["achieved_today"]:
            ...
            if user["messages_today"] >= MESSAGES_REQUIRED:
                ...
                await message.channel.send(
                    f"مبروك يا اسطورة {message.author.mention} 🔥\n"
                    f"وصلت الستريك رقم **{user['streak']}** 🔥",
                    view=StreakInfoView(),
                )
            else:
                save_data(data)
